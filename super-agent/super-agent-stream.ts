...
const client = new A2AClient(`http://localhost:${process.env.SUPER_AGENT_PORT}/`);

async function streamTask(userMessage: string) {
...
rl.question("Enter a message to send to the Super Agent: ", (answer) => {
  rl.close();
  streamTask(answer);
});
